...
@Component({
  ...
  imports: [
    ...
    MatAutocompleteModule, MatTableModule, MatCardModule
  ],
...
export class SaleForm implements OnInit {
  ...
  addItem(item: Item): void {
    ...
    const existing = this.itemsArray.controls.findIndex(
      c => c.get('itemId')?.value === item.id);
    ...
    if (existing >= 0) {
      ...
      if (newQty > item.quantity) {
        this.snackBar.open(
          `Only ${item.quantity} in stock`, 'Close', { duration: 2000 });
        return;
      }
      qty?.setValue(newQty);
    } else {
  ...
  updateQty(index: number, delta: number): void {
    ...
    if (newQty > max) {
      this.snackBar.open(`Only ${max} in stock`, 'Close',
        { duration: 2000 });
      return;
    }
  ...
  get totalAmount(): number {
    return this.itemsArray.controls.reduce((sum, c) =>
      sum + (c.get('sellingPrice')?.value * c.get('quantity')?.value || 0), 0);
  }
  ...
  onSubmit(): void {
    if (this.form.invalid || this.itemsArray.length === 0) {
      if (this.itemsArray.length === 0) {
        this.snackBar.open('Add at least one item to sell',
          'Close', { duration: 3000 });
      }
    ...
    this.saleService.createSale(payload).subscribe({
      next: (sale) => {
        this.snackBar.open(
          `Sale ${sale.invoiceNumber} completed! ₹${sale.totalAmount}`,
          'Close', { duration: 4000 });
        this.router.navigate(['/sales']);
      },
      error: (err) => {
        this.snackBar.open(
          err?.error?.error || 'Failed to complete sale',
          'Close', { duration: 4000 });
        this.isSaving = false;
      }
    });
